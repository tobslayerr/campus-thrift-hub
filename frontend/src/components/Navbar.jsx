...
import { 
  ...
  PlusCircle, LogOut, Bell, Loader2, Heart, BarChart2 
} from 'lucide-react';
...
export default function Navbar() {
  ...
  // Sembunyikan FAB di halaman tertentu agar tidak menghalangi
  ...
  return (
    <>
      ...
      <nav className="bg-white/80 backdrop-blur-md py-3 md:py-4 px-4 md:px-6 sticky top-0 z-[60] shadow-sm border-b border-slate-200">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <Link to="/" className="relative group shrink-0">
            ...
            <img src="/iconweb.png" alt="Logo" className="h-12 md:h-24 w-auto object-contain relative transition-transform duration-500 ease-out group-hover:scale-110 group-hover:-rotate-2" />
          </Link>
          ...
          <div className="md:hidden flex items-center gap-1.5 shrink-0">
              {user && (
                  <>
                      ...
                      <Link to={`/seller/${userId}`} className="p-0.5 rounded-full border-2 border-slate-200 hover:border-[#00478F] active:scale-90 transition-all shrink-0">
                          <img src={user.profilePicture || 'https://via.placeholder.com/150'} alt={user.name} className="w-8 h-8 rounded-full object-cover" />
                      </Link>

                      <Link to="/chats" className="p-2 text-[#00478F] bg-slate-100 rounded-xl active:scale-90 transition-transform shrink-0">
                          <MessageSquare size={20} />
                      </Link>
                      
                      <Link to="/notifications" className="relative p-2 text-[#00478F] bg-slate-100 rounded-xl active:scale-90 transition-transform shrink-0">
                          <Bell size={20} />
                          {unreadCount > 0 && (
                              <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[9px] font-black w-4 h-4 rounded-full flex items-center justify-center animate-bounce shadow-sm border border-white">
                                  {unreadCount > 9 ? '9+' : unreadCount}
                              </span>
                          )}
                      </Link>
              ...
              <button onClick={toggleSidebar} className="p-2 rounded-xl bg-slate-100 text-[#00478F] active:scale-90 transition-transform ml-0.5 shrink-0">
      ...
      <aside className={`fixed top-0 right-0 h-full w-[300px] bg-white z-[80] shadow-2xl transform transition-all duration-500 ease-in-out md:hidden flex flex-col ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-5 flex justify-between items-center border-b border-slate-50 bg-slate-50">
          <img src="/iconweb.png" alt="Logo" className="h-10 w-auto" />
        ...
        <div className="p-5 flex flex-col gap-4 flex-1 overflow-y-auto">
          {user ? (
            <>
              {/* Header Profil Singkat Sidebar */}
              <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 mb-2 text-center flex flex-col items-center relative overflow-hidden group">
                <Link to={`/seller/${userId}`} onClick={toggleSidebar} className="absolute inset-0 z-10"></Link>
                <img src={user.profilePicture || 'https://via.placeholder.com/150'} className="w-20 h-20 rounded-full object-cover ring-4 ring-white shadow-xl mb-4 group-hover:scale-105 transition-transform" alt="" />
                <p className="text-xl font-black text-slate-900 group-hover:text-[#00478F] transition-colors">{user.name}</p>
                <p className="text-xs font-bold text-slate-400 mt-1">Lihat Profil</p>
              </div>
              ...
              <NavLink to="/seller/dashboard" onClick={toggleSidebar} className={navItemStyle}>
                <BarChart2 size={22} /> <span>Analitik Toko</span>
              </NavLink>
              ...
              <NavLink to="/upload" onClick={toggleSidebar} className="flex items-center justify-center gap-3 px-6 py-4 mt-2 bg-[#FF9500] text-white font-black rounded-2xl shadow-lg shadow-orange-200 active:scale-95 transition-transform">
      ...
      {/* FLOATING ACTION BUTTON PILL (+ JUAL BARANG) */}
      {shouldShowFab && (
          <Link 
              to="/upload" 
              className="md:hidden fixed bottom-6 right-6 z-[90] bg-[#FF9500] text-white px-5 h-14 rounded-full shadow-[0_10px_25px_-5px_rgba(255,149,0,0.5)] flex items-center justify-center gap-2 border-4 border-white hover:scale-105 active:scale-95 transition-all duration-300"
              title="Jual Barang Sekarang"
          >
              <PlusCircle size={22} className="shrink-0" />
              <span className="font-black text-sm tracking-wide">Jual Barang</span>
          </Link>
      )}
    </>
  );
